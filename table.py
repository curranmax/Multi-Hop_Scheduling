...
def getMetric(inpt, output, metric = 'percent_packets_delivered'):
	...
	if metric == 'computation_duration':
		return output.computation_duration
...
if __name__ == '__main__':
	data = runner.readDataFromFile('data/8-7/reconfig_delta.txt')

	print data
	dict_table = {}
	for inpt, output_by_method in data:
		for output_name, output in output_by_method.iteritems():
			dict_table[(inpt.reconfig_delta, output_name)] = (getMetric(inpt, output, metric = 'percent_packets_delivered'), getMetric(inpt, output, metric = 'computation_duration'))

	list_table = [list(ks) + list(dict_table[ks]) for ks in sorted(dict_table)]

	print tabulate.tabulate(list_table, headers = ['Reconfig', 'Method', '% Packets Delivered', 'Computation Time(s)'])
